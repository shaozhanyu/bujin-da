...
void TIM2_IRQHandler(void)
{
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
	{
		...
		if(2==fangwei_xch_dir)
		{
				...
				{
					fw_currentPos +=  (ACCEL_TIME_COUNT_VALUE/t5_pwm_value)*MM_PER_STEP;
					
				}
				else if ((fangwei_yundong_daowei_flag == 0) && (fw_ms_count_temp < 0))
				{
					fw_currentPos -=  (ACCEL_TIME_COUNT_VALUE/t5_pwm_value)*MM_PER_STEP; //负方向
				}			
...
{ 
		...
		{
			fy_currentPos += ONE_MS_TIME_COUNT_VALUE*FUYANG_MM_PER_STEP;
		...
		if ((fangwei_yundong_daowei_flag == 0) && (fw_ms_count_temp > 0)) //方位电机向螺杆伸出的方向运动
		{
			fw_currentPos +=  ONE_MS_TIME_COUNT_VALUE*MM_PER_STEP;
