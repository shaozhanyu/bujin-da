...
double fy_currentPos; //当前位置
double fw_currentPos;
double last_fy_currentPos;
double last_fw_currentPos;
///////////////////////////////////////////////////////
...
int main(void)
{
	...
	while (1)
	{
		...
		if (guc_10mscnt >= 2) //时间越短调节的越快，加速度过程越好
		{
				...
				//////////////////////方位电机限位触发////////////////////////////////////
				if ((0 == Fangwei_Lmt1) && (fw_dir == 0)) //gb_SHUN_NI1必须判断，因为只有电机先向电机出轴最短的方向运动然后再向0位运动才进这段程序否则程序会一直进这一段程序，导致电机无法伸出
				{
						TIM_Cmd(TIM5, DISABLE);	 //电机停止
						fw_dir = 2;				 //标志着电机处于停止状态
						fangwei_yundong_daowei_flag = 1; //表示电机处于停止状态
						fw_ms_count_temp=0;
						fw_currentPos = 0;				 //表示方位电机的出轴长度实际为0
						fangwei_TargetPos=0;
						if (fw_reset_flag == 1)
						{
						...
						{
								FW_DIR = 1;				 //推杆向外伸出，方向使能
								fw_dir = 1;				 //方位电机逆转
								fw_timecnt = 0;				 //方位电机计时器开始
								fw_currentPos = 0;				 //表示电机的出轴长度
								fangwei_yundong_daowei_flag = 0; //表示电机处于运动状态
								fw_reset_flag = 1;				 //退出电机复位的标志
								fangwei_TargetPos = 15.831697;	 //这是0位标志,复位后，开始归0运动
								
								fw_Init_flag = 1;
								last_fw_currentPos = 0;

								fw_ms_count_temp = fangwei_TargetPos*STEP_COUNT_PER_MM/ONE_MS_TIME_COUNT_VALUE; //PWM输出持续ms数
				...
				//////////////////////俯仰电机限位触发////////////////////////////////////
				if ((0 == Fuyang_Lmt1) && (fy_dir == 0)) 
				{
						TIM_Cmd(TIM4, DISABLE);
						fuyang_yundong_daowei_flag = 1;
						fy_ms_count_temp=0;
						fy_dir = 2;
						fy_currentPos = 0;//表示俯仰电机的出轴长度实际为0
						fuyang_TargetPos = 0;
						fy_timecnt = 0;
						if (fy_reset_flag == 1)
						{
						...
						else  if (fy_reset_flag == 0)//判断当前是要求俯仰电机归0
						{
								...
								fy_currentPos = 0;
								fy_Init_flag = 1;
								fuyang_TargetPos = 22.091; 
								
								
								fy_Init_flag = 1;
								fy_TargetDeg = 0;
								last_fy_currentPos = 0;
								fy_ms_count_temp = fuyang_TargetPos*FUYANG_STEP_COUNT_PER_MM/ONE_MS_TIME_COUNT_VALUE; //PWM输出持续ms数
			...
			if (Yuntai_yingdafangshi_flag == 0)
			{
				...
				if (Yuntai_fuyang_shezhi_chenggong_flag == 1)
				{
						...
						if (fuyang_yundong_daowei_flag == 1)
						{
							...
							last_fy_currentPos = fy_currentPos;
						}
						if(fabs(fuyang_TargetPos - fy_currentPos) >= 0.002)
							fuyang_xiangdui_TargetPos = fuyang_TargetPos - fy_currentPos;
						else
							fuyang_xiangdui_TargetPos = 0;

						fy_ms_count_temp = fuyang_xiangdui_TargetPos*FUYANG_STEP_COUNT_PER_MM/ONE_MS_TIME_COUNT_VALUE; //PWM输出持续ms数
						...
						if (fy_ms_count_temp == 0)
						{
							...
							fy_timecnt = 0;
							fy_currentPos = fuyang_TargetPos;
				...
				if (Yuntai_fangwei_shezhi_chenggong_flag == 1)
				{
						...
						{
							fw_timecnt = 0;
							last_fw_currentPos = fw_currentPos;
						}
						if(fabs(fangwei_TargetPos - fw_currentPos) >= 0.002)
							fangwei_xiangdui_TargetPos = fangwei_TargetPos - fw_currentPos;
						else
							fangwei_xiangdui_TargetPos = 0;
						//fw_ms_count_temp = (int)((fangwei_xiangdui_TargetPos * 5 / 2 / 0.00635)); //38厘米的杆，一个整步0.00635
						//计算运动指定长度，需要1ms中断几次。PWM是51.2次脉冲/ms ，脉冲当量20157.48/mm,需要393.6ms，10mm运动就需要3936ms。中断就是10次
						fw_ms_count_temp = fangwei_xiangdui_TargetPos*STEP_COUNT_PER_MM/ONE_MS_TIME_COUNT_VALUE;
						if (fw_ms_count_temp == 0)
						{
							...
							fw_currentPos = fangwei_TargetPos;
...
int  Yuntai_kongzhi()
{
		if (Yuntai_kongzhi_flag == 1) //接收到云台控制指令
		{
			...
			if (Yuntai_fuwei_flag == 1) //复位命令
			{
				...
				FW_DIR = 0;														//和俯仰的方向相同，方位电机的DIR为0时电机向零位靠近
				fw_dir = 0;														//标志电机正转
			...
			if (Yuntai_tingzhi_flag == 1)//停止指令
			{
				...
				if ((fw_reset_flag == 1) && (fy_reset_flag == 1))//判断当前不是复位运动中,否则不允许手动停止
				{
					if (fw_ms_count_temp > 0) //停止时算的方位角度
					{
						...
						fw_cos_value = (double)(((57.2713 * 57.2713) + (120.6785 * 120.6785) - (87.625 + fw_currentPos) * (87.625 + fw_currentPos)) / (2 * 57.2713 * 120.6785)); //计算出具体的物理角度实际角度没有乘以100
						gul_jiaodu1 = 1500 - (acos(fw_cos_value) * 18000 / 3.14159265 - 4389.97573);
						fw_TargetDeg = (short)(gul_jiaodu1); //当前方位目标
						gul_fangwei_jiaodu = fw_TargetDeg;	 //当前目标角度
						fangwei_TargetPos = fw_currentPos;			 //方位目标位置
					...
					else if (fw_ms_count_temp < 0)
					{
						...
						fangwei_yundong_daowei_flag =1;
																				  //方位负方向
						fw_cos_value = (double)(((57.2713 * 57.2713) + (120.6785 * 120.6785) - (87.625 + fw_currentPos) * (87.625 + fw_currentPos)) / (2 * 57.2713 * 120.6785)); //计算出具体的物理角度
						gul_jiaodu1 = 1500 - (acos(fw_cos_value) * 18000 / 3.14159265 - 4389.97573);																				  //方位角度
						fw_TargetDeg = (short)(gul_jiaodu1);																												  //乘以100
						gul_fangwei_jiaodu = fw_TargetDeg;
						fangwei_TargetPos = fw_currentPos;
					...
					if (fy_ms_count_temp > 0)
					{
						...
						fy_cos_value = (double)(((553.5480 * 553.5480) + (409.1304 * 409.1304) - (229.625 + fy_currentPos) * (229.625 + fy_currentPos)) / (2 * 553.5480 * 409.1304)); //计算出具体的物理角度
									
						gul_jiaodu = acos(fy_cos_value) * 180 / 3.14159265 - 21.6219;
						fy_TargetDeg = (short)(gul_jiaodu * 100); //停止的时候算俯仰目标角度
						gul_fuyang_jiaodu = fy_TargetDeg;		 //停止的时候算俯仰目标角度
						fuyang_TargetPos = fy_currentPos;
					...
					else if (fy_ms_count_temp < 0)
					{
						TIM_Cmd(TIM4, DISABLE);
						fuyang_yundong_daowei_flag =1;
						//fy_currentPos = last_fy_currentPos - (fy_timecnt * 0.00635 * 2 / 5);
						fy_cos_value = (double)(((553.5480 * 553.5480) + (409.1304 * 409.1304) - (229.625 + fy_currentPos) * (229.625 + fy_currentPos)) / (2 * 553.5480 * 409.1304)); //计算出具体的物理角度
						gul_jiaodu = acos(fy_cos_value) * 180 / 3.14159265 - 21.6219;
						fy_TargetDeg = (short)(gul_jiaodu * 100); //停止的时候算俯仰目标角度
						gul_fuyang_jiaodu = fy_TargetDeg;		 //停止的时候算俯仰目标角度
...
void jiaodu_send()
{
	...
	fw_cos_value = (double)(((57.2713 * 57.2713) + (120.6785 * 120.6785) - (87.625 + fw_currentPos) * (87.625 + fw_currentPos)) / (2 * 57.2713 * 120.6785)); //计算出具体的物理角度
	gul_fangwei_jiaodu = (short)(1500 - (acos(fw_cos_value) * 18000 / 3.14159265 - 4389.97574));//方位角度

	fy_cos_value = (double)(((553.5480 * 553.5480) + (409.1304 * 409.1304) - (229.625 + fy_currentPos) * (229.625 + fy_currentPos)) / (2 * 553.5480 * 409.1304)); //计算出具体的物理角度						
	gul_fuyang_jiaodu = (short)(acos(fy_cos_value) * 18000 / 3.14159265 -2501.9);//俯仰角度
...
void AT24c256_dta_Anal()
{
	...
	if (flag == 2)
	{
		...
		fangwei_Targethudu = (double)((1500 - fw_TargetDeg + 4389.97574) * 3.14159265 / 18000);
		...
		if ((fangwei_TargetPos_temp >= 0) && (fangwei_TargetPos_temp <= 38))
		{
			...
			fw_currentPos = fangwei_TargetPos;
			last_fw_currentPos = fangwei_TargetPos;
			fangwei_yundong_daowei_flag = 1; //到位完成了
		}
		fy_TargetDeg = (short)((AT240c_str[6] << 8) | AT240c_str[7]);
		fuyang_Targethudu = (double)((fy_TargetDeg-500.0 + 2162.1881) * 3.14159265 / 18000);
		...
		if ((fuyang_TargettPos_temp >= 0) && (fuyang_TargettPos_temp <= 58))
		{
			fuyang_TargetPos = fuyang_TargettPos_temp;
			fy_currentPos = fuyang_TargetPos;
			last_fy_currentPos = fuyang_TargetPos;
